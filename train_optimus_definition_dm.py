...
from optimus.data_access.definitions import ExplanationSemanticRoleCorpus, ExplanationInferenceCorpus, DefinitionSemanticRoleCorpus
...
def load_optimus(args, logger):
    ...
    if args.use_pretrained_optimus:
        ...
        model_encoder = encoder_model_class.from_pretrained('cache/bert', cache_dir=None, config=encoder_config, latent_size=args.latent_size) # ,from_tf=bool('.ckpt' in 'bert-base-cased')

        decoder_config = decoder_config_class.from_pretrained('gpt2')
        model_decoder = decoder_model_class.from_pretrained('cache/gpt2', cache_dir=None, config=decoder_config, latent_size=args.latent_size, latent_as_gpt_emb=False, latent_as_gpt_memory=True) # from_tf=bool('.ckpt' in 'gpt2')
...
def train_func(args, model, train_batches=None, test_batches=None, logger=None):
    ...
    for epoch in range(args.epochs):
        model.train()
        # for definition modelling task, only fine-tune the encoder.
        # ---------------------fix or update parameter--------------------------------------------------------
        for l, param in model.named_parameters():
            if l.startswith("decoder"):
...
if __name__ == '__main__':
    ...
    dic = {
        ...
        'eval_dis': False,
        'eval_interval':1,
        'exp':'exp4_train', # definition modelling: exp4_train language modelling: exp1
        'input_eval':None,
        'input_train':None,
        'lambda_adv':0,
        'lambda_kl':0,
        ...
        'pt_lm':'t5-small',
        'save_dir': 'checkpoint_epoch50_beta_0_wikipedia',
        'seed':0,
        'w2v_weights':None
    }
