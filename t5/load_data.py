...
class SetSizeLineByLineTextDataset(Dataset):
    ...
    def __init__(
        ...
        if task == 'recon':
            ...
            # change here !!!!!!!!!!
            max_len = 0
            for text in tqdm(seq_texts):
                # remove blank space.
                t0 = text[0].strip().split(" ")
                text_0 = ' '.join(t0) # </s> is the special token in T5.
                c_p = []
                for i, s in enumerate(text_0.split('[SEP]')):
                    s = s.strip()
                    c_p += [c for c in list(s) if c.strip() != '']
                    if i == len(text_0.split('[SEP]')) - 1:
                        pass
                    else:
                        c_p += ['[SEP]']
                p = tokenizer.convert_tokens_to_ids(c_p)
                size = len(p)
                if max_len < size:
                    max_len = size

            max_len += 1 # add </s>
            bos_eos_id = tokenizer.convert_tokens_to_ids('</s>')
            ...
            for text in tqdm(seq_texts):
                ...
                p = tokenizer.convert_tokens_to_ids(c_p)
                if 2 in p: exit('convert tokens_to_ids suffered from unknown token.')

                input_ids = p + [tokenizer.pad_token_id for _ in range(max_len - len(p))]
                label_input_ids = [bos_eos_id] + p + [tokenizer.pad_token_id for _ in range(max_len - len(p))]
                label_output_ids = p + [bos_eos_id] + [tokenizer.pad_token_id for _ in range(max_len - len(p))]
                ...
                self.examples.append({
                    ...
                    'label_ids': torch.tensor(label_input_ids),
                    'label1_ids': torch.tensor(label_output_ids),
                    'srl': torch.tensor(dimention_inf), # used in VQVAE
                    'srl_ids': torch.tensor(expand_label) # used in input embedding.
                })
            ...
            print("math inference task")
            seq_texts = self._get_text_sequences_rec(file_path)

            random.shuffle(seq_texts)
            self.examples = []

            def math_symbol_inf_data_process(premise, conclusion):
                formula, var = premise.split('where the variable is')
                var = " where [SEP] the [SEP] variable [SEP] is [SEP] " + var
                if 'integration' in formula:
                    pure_formula = formula.split('performing integration for')[-1]
                    operation = "performing [SEP] differentiation [SEP] for "
                elif 'differentiation' in formula:
                    pure_formula = formula.split('performing differentiation for')[-1]
                    operation = "performing [SEP] differentiation [SEP] for "
                else:
                    pure_formula = None
                    operation = None
                    exit("wrong format")
                text_0 = operation + "[SEP] " + pure_formula.strip() + " [SEP]" + var

                formula = conclusion.split('the result is')[-1]
                text_1 = "the [SEP] result [SEP] is [SEP] " + formula.strip()
                return text_0, text_1

            max_len_premise, max_len_conclusion = 0, 0
            for text in tqdm(seq_texts):
                premise, conclusion = math_symbol_inf_data_process(text[0], text[1])
                c_p = []
                for i, s in enumerate(premise.split('[SEP]')):
                    s = s.strip()
                    c_p += [c for c in list(s) if c.strip() != '']
                    if i == len(premise.split('[SEP]')) - 1:
                        pass
                    else:
                        c_p += ['[SEP]']
                p = tokenizer.convert_tokens_to_ids(c_p)
                size = len(p)
                if max_len_premise < size:
                    max_len_premise = size

                c_p = []
                for i, s in enumerate(conclusion.split('[SEP]')):
                    s = s.strip()
                    c_p += [c for c in list(s) if c.strip() != '']
                    if i == len(conclusion.split('[SEP]')) - 1:
                        pass
                    else:
                        c_p += ['[SEP]']
                p = tokenizer.convert_tokens_to_ids(c_p)
                size = len(p)
                if max_len_conclusion < size:
                    max_len_conclusion = size

            max_len_premise += 1 # add </s>
            max_len_conclusion += 1
            ...
            for text in tqdm(seq_texts):
                ...
                if 2 in p:
                    exit('convert tokens_to_ids suffered from unknown token.')

                input_ids = p + [tokenizer.pad_token_id for _ in range(max_len_premise - len(p))]
                ...
                # input_ids = p + [tokenizer.pad_token_id for _ in range(max_len - len(p))]
                label_input_ids = [bos_eos_id] + p + [tokenizer.pad_token_id for _ in range(max_len_conclusion - len(p))]
                label_output_ids = p + [bos_eos_id] + [tokenizer.pad_token_id for _ in range(max_len_conclusion - len(p))]
