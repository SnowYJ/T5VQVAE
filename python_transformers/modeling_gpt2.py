...
class GPT2ForLatentConnector(GPT2PreTrainedModel):
    ...
    """
    def __init__(self, config, latent_size=32, latent_as_gpt_emb=False, latent_as_gpt_memory=True, vocab_role_size=1):

        super(GPT2ForLatentConnector, self).__init__(config, vocab_role_size)

        self.transformer = GPT2Model(config, latent_size,vocab_role_size)
        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
    ...
    def forward(self, input_ids, past=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        if labels is not None:
            ...
            loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
            # print(shift_logits.view(-1, shift_logits.size(-1)).shape) # [25, 50260]
            # print(shift_labels.view(-1).shape) # [25]
            # print(loss.view(-1, shift_labels.shape[-1])) # [5, 5] the loss of label_ignore is 0.
            loss = torch.sum(loss.view(-1, shift_labels.shape[-1]), -1) # [5] batch size
            outputs = (loss,) + outputs
