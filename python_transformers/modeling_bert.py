...
class BertForLatentConnector(BertPreTrainedModel):
    ...
    """
    def __init__(self, config, latent_size, vocab_role_size=1):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, is_role=False, role_ids=None):
        if attention_mask is None:
        ...
        if is_role:
            # exp 2
            embedding_output = self.role_type_embeddings(input_ids)
            position_ids = torch.arange(input_ids.shape[1], dtype=torch.long, device=input_ids.device)
            position_ids = position_ids.unsqueeze(0).expand_as(input_ids)
            embedding_output += self.embeddings.position_embeddings(position_ids)
        else:
            if role_ids is None:
                ...
                # exp 3
                embedding_output = self.embeddings(input_ids, position_ids=position_ids, token_type_ids=token_type_ids) + \
                                   self.role_type_embeddings(role_ids)
