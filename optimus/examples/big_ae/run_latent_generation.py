...
from optimus.pytorch_transformers import BertForLatentConnector, BertTokenizer
from optimus.pytorch_transformers import GPT2Config, OpenAIGPTConfig, XLNetConfig, TransfoXLConfig, BertConfig
from optimus.pytorch_transformers import GPT2Tokenizer, GPT2ForLatentConnector
...
def text_from_latent_code(latent_z, model_vae, args, tokenizer_decoder, cvae=False):
    past = latent_z
...
def interpolate(model_vae, tokenizer_encoder, tokenizer_decoder, args, control=None):
    cvae = False
    ...
    if control:
        latent_z3, coded_length3 = latent_code_from_text(control, tokenizer_encoder, model_vae, args)
    result = []

    num_steps = args.num_interpolation_steps + 1
    for step in range(num_steps+1):
        print("t: ", step * 1.0/num_steps)
        latent_z = latent_z1 + (latent_z2 - latent_z1) * step * 1.0/num_steps

        if control:
            latent_z += latent_z3
        
        text_interpolate = text_from_latent_code(latent_z, model_vae, args, tokenizer_decoder, cvae)
        result.append(text_interpolate)
...
def main():
    ...
    checkpoint = torch.load(os.path.join(output_full_dir, 'training.bin'), map_location='cpu')
