...
def traversal(input_text, dim, top_k):
    ...
    attention_mask = input.ne(model_t5.config.pad_token_id).long()
    # input_embed = torch.tensor(model_t5.tokenizer.encode(' '.join(t0))).unsqueeze(0)
    input_embed = model_t5.t5_model.encoder.get_hidden(input, attention_mask=attention_mask, latent_num_layer=6)
    ...
    for n, i in enumerate(idx):
        ...
        # TODO: after reconstruct latent embedding, feeding into get_encoder_embed.
        encoder_output = model_t5.t5_model.encoder.get_encoder_embed(input_ids=input, attention_mask=attention_mask, vqvae_hidden_states=quantized, latent_num_layer=6)[0]
        pred_con = text_from_latent_code(model_t5, encoder_output, args=args)
...
if __name__ == '__main__':
    ...
    model_dict = {'model_path': '', 't5_model_name': "t5-base", 'model_type': 't5', 'config_name': None,
                  ...
                  'latent_num_layer': 6,
                  'two_stage_vqvae': True,
                  'pretrain_model_path':'checkpoints/vqvae/t5_base_rec_vqvae_layer_6_loss_0.91_45_10000/train'}

    data_dict = {'train_data_file': 'datasets/full/explanations_tr.txt',
                 'test_data_file': 'datasets/full/explanations_te.txt',
    ...
    inf = "an animal is a kind of consumer </s> an animal / bacterium requires oxygen for survival / to breathe"
    exp = "an animal requires warmth in cold environments"
    # for i in range(10):
    #     traversal(exp, dim=i, top_k=20)
    # exit()
    ...
    test_dataloader = data.get_dataloader(args, test_dataset[:100])
    ...
    with torch.no_grad():
        for step, inputs in enumerate(test_dataloader):
            ...
            # quantized = torch.matmul(encodings, model_t5.vqvae._embedding.weight).view(input_shape)
            attention_mask = input.ne(model_t5.config.pad_token_id).long()
            quantized = model_t5.t5_model.encoder(input_ids=input, attention_mask=attention_mask, latent_num_layer=args.latent_num_layer, vqvae=model_t5.vqvae)[0]
